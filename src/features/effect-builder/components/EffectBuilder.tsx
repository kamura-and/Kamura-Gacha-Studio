...
import {
  SaveEffectDialog,
  type EffectSaveValues,
} from "./SaveEffectDialog";
import { Timeline } from "./Timeline";

type EffectBuilderProps = {
  registry: ActionRegistry;

  initialEffect?: EffectDefinition;
...
export function EffectBuilder({
  ...
  const [
    currentEffect,
    setCurrentEffect,
  ] = useState<EffectDefinition | null>(
    initialEffect ?? null,
  );
  ...
  useEffect(() => {
    setCurrentEffect(initialEffect ?? null);
    setTimelineItems(
      initialEffect?.actions ?? [],
    );
    setSelectedId(null);
        ...
        <header className="flex flex-col gap-4 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <Save size={16} />
